/**
 * ProjectsPanel component - Panel for editing projects information
 */
import { bindForm } from '../../utils/reactive-state.js';
import { renderList, createElement, textToList, listToText, formatDate } from '../../utils/helpers.js';

export class ProjectsPanel extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: 'open' });
    this._store = null;
    this._projectsContainer = null;
    this._currentEditIndex = -1;
  }
  
  /**
   * Set the data store and subscribe to changes
   */
  set store(store) {
    this._store = store;
    this.render();
    
    // Subscribe to store changes
    if (store) {
      this._unsubscribe = store.subscribe(this.handleStateChange.bind(this));
    }
  }
  
  get store() {
    return this._store;
  }
  
  connectedCallback() {
    if (this._store) {
      this.render();
    }
  }
  
  disconnectedCallback() {
    // Clean up subscription when element is removed
    if (this._unsubscribe) {
      this._unsubscribe();
    }
  }
  
  render() {
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          display: block;
        }
        
        .resume-section {
          margin-bottom: 1.5rem;
        }
        
        .resume-section-header {
          display: flex;
          justify-content: space-between;
          align-items: center;
          margin-bottom: 0.5rem;
          padding-bottom: 0.25rem;
          border-bottom: 1px solid var(--border-color, #dee2e6);
        }
        
        .resume-section-title {
          font-weight: 600;
          font-size: 1.2rem;
        }
        
        .form-grid {
          display: grid;
          grid-template-columns: 1fr 1fr;
          gap: 1rem;
        }
        
        .input-group {
          display: flex;
          flex-direction: column;
          margin-bottom: 1rem;
        }
        
        .input-group label {
          margin-bottom: 0.5rem;
          font-weight: 500;
        }
        
        input, textarea, select {
          padding: 0.5rem;
          border: 1px solid var(--border-color, #dee2e6);
          border-radius: 0.25rem;
          font-family: inherit;
          font-size: 1rem;
        }
        
        textarea {
          min-height: 100px;
          resize: vertical;
        }
        
        button {
          padding: 0.5rem 1rem;
          background-color: var(--primary-color, #3a86ff);
          color: white;
          border: none;
          border-radius: 0.25rem;
          cursor: pointer;
          font-family: inherit;
          font-size: 1rem;
          display: inline-flex;
          align-items: center;
          gap: 0.5rem;
        }
        
        button.small-button {
          padding: 0.25rem 0.5rem;
          font-size: 0.8rem;
        }
        
        .button-group {
          display: flex;
          gap: 0.5rem;
          margin-top: 1rem;
        }
        
        .resume-item {
          background-color: white;
          border: 1px solid var(--border-color, #dee2e6);
          border-radius: 0.25rem;
          padding: 1rem;
          margin-bottom: 0.5rem;
        }
        
        .resume-item-header {
          display: flex;
          justify-content: space-between;
          margin-bottom: 0.5rem;
        }
        
        .resume-item-actions {
          display: flex;
          gap: 0.5rem;
        }
        
        .icon-button {
          background: none;
          border: none;
          color: var(--text-color, #333);
          cursor: pointer;
          padding: 0.25rem;
        }
        
        .icon-button:hover {
          color: var(--primary-color, #3a86ff);
        }
        
        .empty-state {
          text-align: center;
          padding: 2rem;
          color: var(--secondary-color, #6c757d);
        }
        
        .modal {
          display: none;
          position: fixed;
          top: 0;
          left: 0;
          width: 100%;
          height: 100%;
          background-color: rgba(0, 0, 0, 0.5);
          z-index: 1000;
          align-items: center;
          justify-content: center;
        }
        
        .modal.show {
          display: flex;
        }
        
        .modal-content {
          background-color: white;
          border-radius: 0.5rem;
          padding: 1.5rem;
          width: 90%;
          max-width: 600px;
          max-height: 90vh;
          overflow-y: auto;
        }
        
        .modal-header {
          display: flex;
          justify-content: space-between;
          align-items: center;
          margin-bottom: 1rem;
        }
        
        .modal-title {
          font-size: 1.2rem;
          font-weight: 600;
          margin: 0;
        }
        
        .modal-close {
          background: none;
          border: none;
          font-size: 1.5rem;
          cursor: pointer;
        }
        
        .date-info {
          font-style: italic;
          color: var(--secondary-color, #6c757d);
        }
        
        .tag-list {
          display: flex;
          flex-wrap: wrap;
          gap: 0.5rem;
          margin-top: 0.5rem;
        }
        
        .tag {
          background-color: var(--section-bg, #f8f9fa);
          border-radius: 1rem;
          padding: 0.25rem 0.75rem;
          font-size: 0.9rem;
        }
        
        .help-text {
          font-size: 0.8rem;
          color: var(--secondary-color, #6c757d);
          margin-top: 0.25rem;
        }
        
        @media (max-width: 768px) {
          .form-grid {
            grid-template-columns: 1fr;
          }
        }
      </style>
      
      <div class="resume-section">
        <div class="resume-section-header">
          <div class="resume-section-title">Projects</div>
          <button id="add-project" class="small-button">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="12" y1="5" x2="12" y2="19"></line>
              <line x1="5" y1="12" x2="19" y2="12"></line>
            </svg>
            Add Project
          </button>
        </div>
        <div id="projects-container"></div>
      </div>
      
      <!-- Project Modal -->
      <div id="project-modal" class="modal">
        <div class="modal-content">
          <div class="modal-header">
            <h3 class="modal-title">Add Project</h3>
            <button class="modal-close">&times;</button>
          </div>
          <form id="project-form">
            <div class="form-grid">
              <div class="input-group">
                <label for="project-name">Project Name</label>
                <input type="text" id="project-name" name="name" required>
              </div>
              <div class="input-group">
                <label for="project-url">URL</label>
                <input type="url" id="project-url" name="url" placeholder="https://example.com/project">
              </div>
              <div class="input-group">
                <label for="project-startDate">Start Date</label>
                <input type="text" id="project-startDate" name="startDate" placeholder="YYYY-MM-DD">
                <span class="help-text">Use format YYYY-MM-DD</span>
              </div>
              <div class="input-group">
                <label for="project-endDate">End Date</label>
                <input type="text" id="project-endDate" name="endDate" placeholder="YYYY-MM-DD or 'Present'">
                <span class="help-text">Use format YYYY-MM-DD or 'Present'</span>
              </div>
            </div>
            <div class="input-group">
              <label for="project-description">Description</label>
              <textarea id="project-description" name="description"></textarea>
            </div>
            <div class="input-group">
              <label for="project-highlights">Highlights (one per line)</label>
              <textarea id="project-highlights" name="highlights" 
                placeholder="• Implemented X feature using Y technology"></textarea>
              <span class="help-text">Enter each achievement on a new line</span>
            </div>
            <div class="input-group">
              <label for="project-keywords">Keywords/Technologies (one per line)</label>
              <textarea id="project-keywords" name="keywords" 
                placeholder="JavaScript, React, Node.js"></textarea>
              <span class="help-text">Enter each keyword on a new line</span>
            </div>
            <div class="button-group">
              <button id="save-project" type="submit" class="primary">Save</button>
              <button type="button" class="modal-cancel secondary">Cancel</button>
            </div>
          </form>
        </div>
      </div>
    `;
    
    // Get references to DOM elements
    this._projectsContainer = this.shadowRoot.querySelector('#projects-container');
    
    // Setup event listeners
    this.setupEventListeners();
    
    // Render projects list
    this.renderProjects();
  }
  
  setupEventListeners() {
    // Add project button
    const addProjectBtn = this.shadowRoot.querySelector('#add-project');
    if (addProjectBtn) {
      addProjectBtn.addEventListener('click', () => this.openProjectModal());
    }
    
    // Project modal setup
    const projectModal = this.shadowRoot.querySelector('#project-modal');
    const closeButtons = projectModal.querySelectorAll('.modal-close, .modal-cancel');
    closeButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        projectModal.classList.remove('show');
      });
    });
    
    // Project form submission
    const projectForm = this.shadowRoot.querySelector('#project-form');
    projectForm.addEventListener('submit', (e) => {
      e.preventDefault();
      this.saveProject();
    });
  }
  
  renderProjects() {
    if (!this._store || !this._projectsContainer) return;
    
    const projects = this._store.state.projects || [];
    
    renderList(
      projects,
      (project, index) => this.renderProjectItem(project, index),
      this._projectsContainer,
      {
        emptyMessage: `
          <div class="empty-state">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" stroke-linecap="round" stroke-linejoin="round">
              <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
              <line x1="3" y1="9" x2="21" y2="9"></line>
              <line x1="9" y1="21" x2="9" y2="9"></line>
            </svg>
            <p>No projects added yet. Click the "Add Project" button to add your notable projects.</p>
          </div>
        `,
        emptyClass: 'empty-state'
      }
    );
  }
  
  renderProjectItem(project, index) {
    const item = createElement('div', { className: 'resume-item' }, [
      createElement('div', { className: 'resume-item-header' }, [
        createElement('strong', {}, project.name),
        createElement('div', { className: 'resume-item-actions' }, [
          createElement('button', { 
            className: 'icon-button',
            dataset: { index },
            onClick: () => this.editProject(index)
          }, [
            createElement('svg', {
              xmlns: 'http://www.w3.org/2000/svg',
              width: '16',
              height: '16',
              viewBox: '0 0 24 24',
              fill: 'none',
              stroke: 'currentColor',
              'stroke-width': '2',
              'stroke-linecap': 'round',
              'stroke-linejoin': 'round'
            }, [
              createElement('path', { d: 'M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7' }),
              createElement('path', { d: 'M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z' })
            ])
          ]),
          createElement('button', { 
            className: 'icon-button',
            dataset: { index },
            onClick: () => this.deleteProject(index)
          }, [
            createElement('svg', {
              xmlns: 'http://www.w3.org/2000/svg',
              width: '16',
              height: '16',
              viewBox: '0 0 24 24',
              fill: 'none',
              stroke: 'currentColor',
              'stroke-width': '2',
              'stroke-linecap': 'round',
              'stroke-linejoin': 'round'
            }, [
              createElement('polyline', { points: '3 6 5 6 21 6' }),
              createElement('path', { d: 'M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2' }),
              createElement('line', { x1: '10', y1: '11', x2: '10', y2: '17' }),
              createElement('line', { x1: '14', y1: '11', x2: '14', y2: '17' })
            ])
          ])
        ])
      ]),
      createElement('div', {}, [
        (project.startDate || project.endDate) && createElement('div', { className: 'date-info' }, [
          formatDate(project.startDate),
          ' - ',
          project.endDate ? (project.endDate.toLowerCase() === 'present' ? 'Present' : formatDate(project.endDate)) : ''
        ]),
        project.url && createElement('div', {}, [
          'URL: ',
          createElement('a', { href: project.url, target: '_blank' }, project.url)
        ]),
        project.description && createElement('div', { style: 'margin-top: 0.5rem;' }, project.description),
        project.highlights && project.highlights.length > 0 && createElement('div', { style: 'margin-top: 0.5rem;' }, [
          createElement('strong', {}, 'Highlights:'),
          createElement('ul', { style: 'margin-top: 0.25rem;' }, 
            project.highlights.map(highlight => 
              createElement('li', {}, highlight)
            )
          )
        ]),
        project.keywords && project.keywords.length > 0 && createElement('div', { style: 'margin-top: 0.5rem;' }, [
          createElement('strong', {}, 'Technologies:'),
          createElement('div', { className: 'tag-list' },
            project.keywords.map(keyword => 
              createElement('span', { className: 'tag' }, keyword)
            )
          )
        ])
      ])
    ]);
    
    return item;
  }
  
  openProjectModal(index = -1) {
    const modal = this.shadowRoot.querySelector('#project-modal');
    const title = modal.querySelector('.modal-title');
    const form = modal.querySelector('#project-form');
    
    // Reset form
    form.reset();
    
    if (index >= 0) {
      // Editing existing project
      const project = this._store.state.projects[index];
      title.textContent = 'Edit Project';
      
      // Fill form with project data
      form.elements.name.value = project.name || '';
      form.elements.url.value = project.url || '';
      form.elements.startDate.value = project.startDate || '';
      form.elements.endDate.value = project.endDate || '';
      form.elements.description.value = project.description || '';
      
      // Convert arrays to text
      form.elements.highlights.value = listToText(project.highlights || []);
      form.elements.keywords.value = listToText(project.keywords || []);
      
      // Store index for later use
      this._currentEditIndex = index;
    } else {
      // Adding new project
      title.textContent = 'Add Project';
      this._currentEditIndex = -1;
    }
    
    // Show modal
    modal.classList.add('show');
  }
  
  saveProject() {
    const form = this.shadowRoot.querySelector('#project-form');
    
    // Get form data
    const name = form.elements.name.value.trim();
    const url = form.elements.url.value.trim();
    const startDate = form.elements.startDate.value.trim();
    const endDate = form.elements.endDate.value.trim();
    const description = form.elements.description.value.trim();
    const highlights = textToList(form.elements.highlights.value);
    const keywords = textToList(form.elements.keywords.value);
    
    // Validate required fields
    if (!name) {
      alert('Project name is required');
      return;
    }
    
    // Create project object
    const newProject = { 
      name, 
      url, 
      startDate, 
      endDate, 
      description, 
      highlights, 
      keywords 
    };
    
    // Update store
    if (this._currentEditIndex >= 0) {
      // Update existing project
      this._store.state.projects[this._currentEditIndex] = newProject;
    } else {
      // Add new project
      if (!Array.isArray(this._store.state.projects)) {
        this._store.state.projects = [];
      }
      this._store.state.projects.push(newProject);
    }
    
    // Hide modal
    const modal = this.shadowRoot.querySelector('#project-modal');
    modal.classList.remove('show');
    
    // Re-render projects list
    this.renderProjects();
  }
  
  editProject(index) {
    this.openProjectModal(index);
  }
  
  deleteProject(index) {
    if (confirm('Are you sure you want to delete this project?')) {
      this._store.state.projects.splice(index, 1);
      this.renderProjects();
    }
  }
  
  handleStateChange(state, change) {
    // Update projects list if projects has changed
    if (change && 
        (change.property === 'projects' || 
         change.property.startsWith('projects.') ||
         (change.reset && state.projects))) {
      this.renderProjects();
    }
  }
}

// Define the custom element
customElements.define('projects-panel', ProjectsPanel);
